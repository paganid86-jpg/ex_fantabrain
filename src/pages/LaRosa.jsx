  ...
  return (
    <div className="glass-elevated" style={{ position: 'sticky', top: 0 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 22, color: 'var(--text-primary)', lineHeight: 1.1 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
        {[
          ...
          <div key={item.label} style={{
            ...
            <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-display)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>
              {item.label}
            </div>
          ...
          {giocatore.votiUltimi5.map((v, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, height: '100%', justifyContent: 'flex-end' }}>
                <div style={{ fontSize: 9, color, fontFamily: 'var(--font-display)', fontWeight: 700 }}>
                  {v > 0 ? v : 'SV'}
                </div>
                <div style={{
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {kpiRuoli.map((k) => (
          <div key={k.ruolo} style={{
            ...
            <span style={{ fontSize: 12, color: 'var(--text-secondary)', fontFamily: 'var(--font-display)' }}>{k.ruolo}</span>
          </div>
        ))}
      </div>
              ...
              <tbody>
                {rosaFiltrata.map((g) => (
                  <tr
                    ...
                    style={{
                      cursor: 'pointer',
                      background: selezionato?.id === g.id ? 'rgba(126, 173, 212,0.06)' : undefined,
                    ...
                    <td onClick={(e) => e.stopPropagation()} style={{ textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                        <button
                          ...
                          style={{
                            background: 'rgba(126, 173, 212,0.12)', border: '1px solid rgba(126, 173, 212,0.25)',
                            borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontSize: 13,
                            color: 'var(--accent-primary)', transition: 'all 0.15s',
                          }}
                          onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(126, 173, 212,0.22)'; }}
                          onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(126, 173, 212,0.12)'; }}
                        >✏️</button>
                        <button
                          title="Rimuovi"
